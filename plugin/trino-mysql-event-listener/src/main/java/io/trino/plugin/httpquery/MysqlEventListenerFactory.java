        ...
        implements EventListenerFactory
{
            ...
            implements Module
    {
        ...
        public void configure(Binder binder)
        {
            ...
            configBinder(binder).bindConfig(MySqlDataSourceConfig.class, "mysql-event-listener");
        }

        @Singleton
        @Provides
        DataSource createDataSource(MysqlEventListenerConfig config, MySqlDataSourceConfig dataSourceConfig)
        {
            ServiceDescriptor descriptor = serviceDescriptor("mysql")
                    .addProperty("jdbc", config.getUrl())
                    .build();
            return new MySqlDataSource(new StaticServiceSelector(descriptor), dataSourceConfig);
        }

        @Singleton
        @Provides
        public ConnectionFactory createConnectionFactory(DataSource dataSource)
        {
