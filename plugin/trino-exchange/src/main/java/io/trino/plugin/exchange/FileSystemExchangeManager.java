        ...
        implements ExchangeManager
{
    ...
    public FileSystemExchangeManager(FileSystemExchangeStorage exchangeStorage, FileSystemExchangeConfig fileSystemExchangeConfig)
    {
        ...
        try {
            this.baseDirectory = new URI(baseDirectory);
        }
        catch (URISyntaxException e) {
            throw new IllegalArgumentException("Invalid URI: " + baseDirectory, e);
        }
    ...
    public ExchangeSink createSink(ExchangeSinkInstanceHandle handle)
    {
        ...
        return new FileSystemExchangeSink(
                exchangeStorage,
                fileSystemExchangeSinkInstanceHandle.getOutputDirectory(),
                fileSystemExchangeSinkInstanceHandle.getOutputPartitionCount(),
                fileSystemExchangeSinkInstanceHandle.getSinkHandle().getSecretKey());
    }
    ...
    public ExchangeSource createSource(List<ExchangeSourceHandle> handles)
    {
        ...
        Optional<SecretKey> secretKey = Optional.empty();
        if (!handles.isEmpty()) {
            checkArgument(handles.stream()
                    .map(FileSystemExchangeSourceHandle.class::cast)
                    .map(FileSystemExchangeSourceHandle::getSecretKey)
                    .distinct().count() == 1,
                    "Secret key is expected to be the same");
            secretKey = ((FileSystemExchangeSourceHandle) handles.get(0)).getSecretKey();
        }
        return new FileSystemExchangeSource(exchangeStorage, files, secretKey);
    }
}
