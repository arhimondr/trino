        ...
        extends AbstractConfigurationAwareModule
{
    ...
    protected void setup(Binder binder)
    {
        ...
        configBinder(binder).bindConfig(FileSystemExchangeConfig.class);

        FileSystemExchangeConfig fileSystemExchangeConfig = buildConfigObject(FileSystemExchangeConfig.class);

        String scheme;
        try {
            scheme = new URI(fileSystemExchangeConfig.getBaseDirectory()).getScheme();
        }
        catch (URISyntaxException e) {
            throw new IllegalArgumentException("Invalid URI: " + fileSystemExchangeConfig.getBaseDirectory(), e);
        }
