        ...
        implements NodeAllocator
{
    ...
    private synchronized Optional<InternalNode> tryAcquireNode(NodeRequirements requirements)
    {
        ...
        List<InternalNode> nodesMatchingRequirements = nodes.stream()
                .filter(node -> requirements.getAddresses().isEmpty() || requirements.getAddresses().get().contains(node.getHostAndPort()))
        ...
        int allocationCount = allocationCountMap.compute(selectedNode.get(), (key, value) -> value == null ? 1 : value + 1);
        checkState(allocationCount <= maximumAllocationsPerNode, "allocation count for node %s is expected to be less than %s: %s", selectedNode.get(), maximumAllocationsPerNode, allocationCount);
        return selectedNode;
    }
