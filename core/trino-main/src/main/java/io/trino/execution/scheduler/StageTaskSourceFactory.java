        ...
        implements TaskSourceFactory
{
    ...
    {
        this(splitSourceFactory, tableExecuteContextManager, requireNonNull(queryManagerConfig, "queryManagerConfig is null").getScheduleSplitBatchSize());
    }
            ...
            implements TaskSource
    {
        ...
        public List<TaskDescriptor> getMoreTasks()
        {
            NodeRequirements nodeRequirements = new NodeRequirements(Optional.empty(), Optional.empty());
            ...
            for (Map.Entry<PlanFragmentId, ExchangeSourceHandle> entry : exchangeSourceHandles.entries()) {
                ...
                ExchangeSourceStatistics exchangeSourceStatistics = sourceExchange.getExchangeSourceStatistics(originalExchangeSourceHandle);

                List<ExchangeSourceHandle> exchangeSourceHandles;
                if (exchangeSourceStatistics.getSizeInBytes() > targetPartitionSizeInBytes) {
                    ...
                    exchangeSourceHandles = sourceHandles.build();
                }
                else {
                    exchangeSourceHandles = ImmutableList.of(originalExchangeSourceHandle);
                }

                for (ExchangeSourceHandle handle : exchangeSourceHandles) {
                    ...
                    while (!splitSource.isFinished()) {
                        ...
                        for (Split split : splitBatch.getSplits()) {
                            ...
                            if (bucketToPartitionMap.isPresent()) {
                                ...
                                verify(partitionedExchangeSourceHandles.isEmpty(), "bucket to partition map is expected to be set when partitioned inputs are present");
                                partition = bucket;
                            }
                    ...
                    if (hostSplits.size() >= targetPartitionSplitCount) {
                        ...
                        return ImmutableList.of(
                                new TaskDescriptor(
                                        ...
                                        replicatedExchangeSourceHandles,
                                        new NodeRequirements(catalogRequirement, Optional.of(ImmutableSet.of(remoteHost)))));
                    }
                }
